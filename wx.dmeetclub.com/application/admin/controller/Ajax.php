...
class Ajax extends Backend
{
    ...
    public function getAreaList()
    {
        ...
        if(!$pid) {
            $list = model('app\common\model\AreaNew')->field('id as value, name')->where('level', 2)->select();
        } else {
            $list = model('app\common\model\AreaNew')->field('id as value, name')->where('pid', $pid)->select();
