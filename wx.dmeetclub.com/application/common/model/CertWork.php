 ...
 * 认证审核 - 工作
 */
class CertWork Extends Model
{

    protected $name = 'cert_work';
    // 开启自动写入时间戳字段
     ...
     * @return mixed|string
     */
    public function getTradeTextAttr($value, $row)
    {
        return Dict::getCertWorkTradeType($row['trade_id']);
    }
    ...
    public function submit(User $user, $data)
    {
        ...
        $userCert =  model('app\common\model\UserCert')
            ...
            ->find();
        if(in_array($userCert->work_status, [
        ...
        //更新工作认证状态
        $userCert->work_status = Dict::CERT_STATUS_WAIT;
        $userCert->cert_reject_work = null;//清空驳回意见
        $userCert->save();
        ...
        try {
            ...
            if($userCert) {
                $userCert->allowField(true)->save([
                    "work_status" => Dict::CERT_STATUS_APPROVE,
                    "company"     => $this->company,
                    "trade_id"    => $this->trade_id,
                    "position"    => $this->position,
                    "work_images" => $this->images,
                ]);
            }
            ...
            if($user) {
                $user->allowField(true)->save(['is_cert_work' => Dict::IS_TRUE]);
            }
            Db::commit();
    ...
    public function auditReject($params)
    {
        ...
        try {
            ...
            if($userCert) {
                $userCert->allowField(true)->save([
                    "work_status" => Dict::CERT_STATUS_REJECT,
                    "cert_reject_work" => $this->reject_reason
                ]);
            }
