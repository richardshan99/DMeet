...
function updateConfigWithOptions(options: QiniuOptions) {
	...
	if (options.regionUrl) {
	    config.qiniuRegionUrl = options.regionUrl;
	} else {
	    console.error('qiniu uploader need your bucket regionUrl');
	}
...
function doUpload(uploadOptions: QiniuUploadOptions) {
	...
	const url = config?.qiniuRegionUrl||uploadURLFromRegionCode('ECN')
    if (!url) {
