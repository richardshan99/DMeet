<template>
	<view class="main">
		...
		<view class="main-base">
			<uni-nav-bar left-icon="left" @clickLeft="navBack" :border="false" :shadow="false" title="关注我的" background-color="transparent" :status-bar="true"></uni-nav-bar>
			<scroll-view type="list" :scroll-y="true" class="scroll" @scrolltolower="loadFollowList">
				<view class="item" v-for="(item,index) in dataList" :key="'follow'+index">
					<view class="item-left">
						<image @click="toUserDetail(item.user.id)" mode="aspectFill" :src="item.user.avatar_text" class="head_icon"></image>
					...
					<view @click="toggleFocus(item.user.id)" v-if="item.follow_type == 1" class="item-focus">
						<view class="child">
							<text class="txt">关注</text>
						</view>
					</view>
					<text @click="toggleFocus(item.user.id)" v-if="item.follow_type == 3" class="item-status">互相关注</text>
				</view>
...
<script lang="ts" setup>
	...
	const getFollowList = async (refresh:boolean) => {
		...
		loading.value = true
		try {
			const res:any = await api.post('my/fans_list',{
				page: pageNo
			})
			loading.value = false
			if (res.code == 1) {
