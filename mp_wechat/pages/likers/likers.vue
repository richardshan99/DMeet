<template>
	<view class="main">
		...
		<view class="main-base">
			...
			<scroll-view type="list" :scroll-y="true" class="scroll" @scrolltolower="loadLikers">
				<view class="item" v-for="(item,index) in dataList" :key="'likes'+index" @click="toUserDetail(item.user.id)">
					<view class="item-left">
						<image mode="aspectFill" :src="item.user.avatar_text" class="head_icon"></image>
						<view :style="{display:'flex',flexDirection:'column',marginLeft:'24rpx',flex:1}">
							<view :style="{display:'flex',flexDirection:'row',alignItems:'center'}">
								<text class="name">{{item.user.nickname}}</text>
								<image v-if="item.user.gender == 1" src="/static/focus/man.png" :style="{width: '28rpx',height:'28rpx',marginLeft:'8rpx'}"></image>
								<image v-else="item.user.gender == 2" src="/static/focus/woman.png" :style="{width: '28rpx',height:'28rpx',marginLeft:'8rpx'}"></image>
							</view>
							<text class="desc">{{item.user.birth_text}}年 · {{item.user.height}}cm · {{item.user?.area}}</text>
						</view>
					</view>
					<view @click.stop="toggleFocus(item.user.id)" v-if="item.follow_type == 1" class="item-focus">
						<view class="child">
							<text class="txt">关注</text>
						</view>
					</view>
					<text @click.stop="toggleFocus(item.user.id)" v-if="item.follow_type == 2" class="item-status">已关注</text>
					<text @click.stop="toggleFocus(item.user.id)" v-if="item.follow_type == 3" class="item-status">互相关注</text>
				</view>
...
<script lang="ts" setup>
	...
	const getLikes = async (refresh:boolean) => {
		if (refresh) {
			pageNo = 1
			finish.value = false
		}
		...
		loading.value = true
		try {
			const res:any = await api.post('my/dynamic/like_user',{
				blog_id: blogId,
				page: pageNo
			})
			loading.value = false
