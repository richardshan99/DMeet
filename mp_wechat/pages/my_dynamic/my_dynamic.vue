<template>
	<view class="main">
		...
		<view class="main-base">
			<uni-nav-bar left-icon="left" @clickLeft="navBack" :border="false" :shadow="false" title="我的动态" background-color="transparent" :status-bar="true"></uni-nav-bar>
			<scroll-view class="scroll" :scroll-y="true" type="list" @scrolltolower="loadDynamic">
				<view class="scroll-base" v-for="(item,ind) in dataList" :key="ind+'dynamic'">
					<dynamic-item @refresh="getDynamics(true)" :isMine="true" :dynamicItem="item"></dynamic-item>
				</view>
		...
		<image @click="publishTrends" src="/static/dynamic/add.png" class="main-add"></image>
	</view>
</template>
...
<script lang="ts" setup>
	...
	const getDynamics = async (refresh:boolean) => {
		...
		loading.value = true
		try {
			const res:any = await api.post('/my/dynamic/list',{
				page: pageNo
			})
			loading.value = false
			if (res.code == 1) {
