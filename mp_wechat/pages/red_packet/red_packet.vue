<template>
  <view class="main">
    ...
    <view class="main-base">
      ...
      <view class="balance">
        <view class="left">
          <text class="balance-txt1">红包余额</text>
          <text class="balance-txt2">{{ balance }}</text>
        </view>
        <view class="right">
          <view class="withdraw" @click="withdraw">提现</view>
        </view>
      </view>
      <scroll-view
        type="list"
...
const getBalanceList = async (refresh: boolean) => {
  ...
  loading.value = true;
  try {
    const res: any = await api.post("my_red_envelope_balance/list", {
      page: pageNo,
    });
    loading.value = false;
    console.log(res);
    ...
    if (res.code == 1) {
      balance.value = res.data.red_envelope_balance; // res.data.balance;
      if (refresh) {
...
const withdraw = () => {
  uni.navigateTo({
    url: `/pages/red_packet_withdraw/red_packet_withdraw?money=${balance.value}`,
  });
};
</script>
...
.main {
  ...
  &-base {
    ...
    .balance {
      width: 686rpx;
      height: 196rpx;
      background: linear-gradient(104deg, #ff3a5a, #ff4f95 100%);
      border-radius: 24rpx;
      margin-top: 24rpx;
      display: flex;
      padding-left: 40rpx;
